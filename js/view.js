...
view.row=function(colspans,model_content_index){
	...
	if (!colspans){var colspans=davis.pick(model.frame);}
	var d="";
	d+="<tr>";
...
view.posthoc=function(){
	...
	$("td:has(h1)").css({"vertical-align":"middle"});

	$("td:empty").each(function(){
		var id=davis.randomWord();
		...
		$(this).css({"background":"url("+davis.pick(model.images)+") no-repeat "+davis.pick(positions)+""});
		}
...
view.graph=function(id){
	...
	if (h>50){
		...
		var dHole=c.circle(x,y,(r/2))
			.attr({"fill":$("body").css("background-color"),"stroke":"#fff"});
	}
};
